...
namespace BBUnity.Actions
{
    ...
    public class SendMessage : GOAction
    {
        ...
        public override void OnStart()
        {
            ...
            for(int i = 0; i < GameObject.FindGameObjectsWithTag("Cop").Where(x => !x.GetComponent<Moves>().found).Count(); i++)
            {

                GameObject.FindGameObjectsWithTag("Cop").Where(x => !x.GetComponent<Moves>().found).ElementAt(i).GetComponent<Moves>().found = true;
